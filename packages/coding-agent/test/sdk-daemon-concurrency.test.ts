...
describe("ConversationStore", () => {
	...
	test("accepts EBADF when a completed lock handle is already closed", async () => {
		class ClosedLockFs extends MemoryConversationStoreFs {
			override async open(file: string, flags: string) {
				...
				return {
					...
					close: async () => {
						await handle.close();
						throw Object.assign(new Error("EBADF: lock handle already closed"), { code: "EBADF" });
					},
				};
			}
		}
		const fs = new ClosedLockFs();
		const store = new ConversationStore<TestConversation>({ agentDir: "/agent", kind: "slack", fs, now: () => 1 });
